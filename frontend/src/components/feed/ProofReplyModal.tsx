...
import { X, Send, Heart, Flame, Shield, CheckCircle2, MessageCircle } from "lucide-react";
...
export const ProofReplyModal: React.FC = () => {
  ...
  const currentComments = proofComments[activeProofForReply.id] || [];

  const handleSend = (e: React.FormEvent) => {
    e.preventDefault();
    if (!commentInput.trim()) return;
    addProofComment(activeProofForReply.id, commentInput.trim());
    setCommentInput("");
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/80 backdrop-blur-sm">
        <motion.div
          ...
          className="w-full max-w-md bg-neutral-950 border-t border-neutral-800 rounded-t-3xl max-h-[85vh] flex flex-col justify-between shadow-2xl text-white"
        >
          {/* Header */}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-neutral-400 font-medium">
                  {activeProofForReply.arenaTag} • {currentComments.length} replies
                </span>
              </div>
            </div>
          ...
          <div className="p-3 bg-neutral-900/40 border-b border-neutral-900 flex items-center gap-3">
            <div className="w-12 h-14 rounded-xl overflow-hidden bg-neutral-800 shrink-0 border border-neutral-700 flex items-center justify-center">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 no-scrollbar min-h-[160px]">
            {currentComments.length === 0 ? (
              <div className="text-center py-8 space-y-1">
                <p className="text-xs font-bold text-neutral-400">No replies yet.</p>
                <p className="text-[10px] text-neutral-600">Be the first to encourage this drop!</p>
              </div>
            ) : (
              currentComments.map((comment) => (
                <div key={comment.id} className="flex items-start gap-2.5">
                  <img
                    ...
                    className="w-7 h-7 rounded-full object-cover shrink-0 border border-neutral-800 mt-0.5"
                  />
                  <div className="flex-1 bg-neutral-900/70 border border-neutral-800/80 rounded-2xl p-2.5">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-[11px] font-bold text-white">
                        {comment.userName}
                      </span>
                      <span className="text-[9px] text-neutral-500">{comment.timeAgo}</span>
                    </div>
                    <p className="text-xs text-neutral-200 leading-relaxed">{comment.text}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="p-3 border-t border-neutral-900 bg-neutral-950 space-y-2">
            ...
            <form onSubmit={handleSend} className="flex items-center gap-2">
              <input
                ...
                autoFocus
                className="flex-1 py-2.5 px-4 rounded-2xl bg-neutral-900 border border-neutral-800 text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-emerald-500/50"
              />
