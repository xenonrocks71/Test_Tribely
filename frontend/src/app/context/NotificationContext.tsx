...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
        ...
        if (data.event_type === "unread_update") {
          ...
          if (data.notification) {
            ...
            setActiveBanner({
              ...
              arenaId: targetArena,
              url: data.notification.data_json?.url || `/arena/${targetArena}`
            });
  ...
  return (
    <NotificationContext.Provider value={{ unreadCounts, markArenaRead, fetchUnreadCounts }}>
      ...
      {/* Floating WhatsApp-Style In-App DM Toast Popup Banner */}
      {activeBanner && (
        <div
        ...
        >
          <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white shrink-0 shadow-sm" style={{ background: activeBanner.title.includes("Call") ? "linear-gradient(135deg, #10B981, #059669)" : "var(--accent-gradient)" }}>
