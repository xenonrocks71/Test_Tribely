...
Enforces 60% Arena Reserve, 30% Single-Winner Reward Pool, and 10% Platform Commission allocation model in direct INR.
...
class LedgerService:
    ...
    def record_penalty_accrual(
        ...
        Records razorpay_payment_id for auditability.
        """
        ...
        commission = round(penalty_amount_inr * COMMISSION_PCT, 2)
        reserve = round(penalty_amount_inr * RESERVE_PCT, 2)
        reward = round(penalty_amount_inr * REWARD_PCT, 2)
        ...
        if existing:
            ...
            return {
                ...
                "arena_pool": {
                    "reserve_pool_inr": pool.reserve_pool_inr,
                    "reward_pool_inr": pool.reward_pool_inr,
        ...
        # 2. Update ArenaPool
        pool = self.get_or_create_arena_pool(db, arena_id)
        pool.reserve_pool_inr += reserve
        pool.reward_pool_inr += reward
    ...
    def get_or_create_arena_pool(self, db: Session, arena_id: int) -> ArenaPool:
        ...
        if not pool:
            pool = ArenaPool(arena_id=arena_id, reserve_pool_inr=0.0, reward_pool_inr=0.0, total_penalties_count=0)
            db.add(pool)
            db.commit()
            db.refresh(pool)
        return pool
    ...
    def get_or_create_user_wallet(self, db: Session, user_id: int) -> UserWallet:
        ...
        if not wallet:
            wallet = UserWallet(user_id=user_id, balance_inr=0.0, last_reward_won_at=None)
            db.add(wallet)
            db.commit()
            db.refresh(wallet)
        return wallet
    ...
    def select_and_award_top_performer(self, db: Session, arena_id: int) -> Dict[str, Any]:
        ...
        Awards reward_pool_inr, credits winner's UserWallet, and resets reward_pool_inr to 0.0.
        """
        pool = self.get_or_create_arena_pool(db, arena_id)
        accumulated_reward = pool.reward_pool_inr
        ...
        for m in memberships:
            ...
            wallet = db.query(UserWallet).filter(UserWallet.user_id == uid).first()
        ...
        if accumulated_reward > 0:
            ...
            winner_wallet.balance_inr += accumulated_reward
            ...
            reward_entry = EscrowLedger(
                ...
                amount_inr=accumulated_reward,
                entry_type="reward_payout",
                idempotency_key=idempotency_key,
                description=f"Single-winner reward payout of INR {accumulated_reward} to user {winner_id}"

            )
            db.add(reward_entry)

            # Reset reward_pool_inr = 0.0
            pool.reward_pool_inr = 0.0
            pool.updated_at = now
            db.commit()
    ...
    def handle_zero_penalty_cycle(self, db: Session, arena_id: int) -> Dict[str, Any]:
        ...
        Handle cycle when zero penalties were accrued:
          ...
          - Deduct commission from ArenaPool.reserve_pool_inr.
        ...
        baseline_stake = float(arena.penalty_amount or 500.0)
        commission = round(baseline_stake * COMMISSION_PCT, 2)

        pool = self.get_or_create_arena_pool(db, arena_id)
        
        # Deduct commission from reserve pool
        pool.reserve_pool_inr = max(0.0, pool.reserve_pool_inr - commission)
        pool.updated_at = datetime.utcnow()
        ...
        ledger_entry = EscrowLedger(
            ...
            idempotency_key=idempotency_key,
            description=f"Zero-penalty cycle 10% platform commission of INR {commission} drawn from reserve pool"
        )
        db.add(ledger_entry)
        db.commit()
        db.refresh(pool)
        ...
        return {
            "status": "success",
            "message": f"Deducted INR {commission} platform commission from arena reserve pool",
            "commission_deducted_inr": commission,
            "remaining_reserve_pool_inr": pool.reserve_pool_inr
        }
